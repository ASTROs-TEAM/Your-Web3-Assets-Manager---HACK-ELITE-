...
const logBalances = () => {
  return new Promise(async (resolve, reject) => {
    let myWalletEthBalance =
      +(await Compound.eth.getBalance(myWalletAddress, providerUrl)) / 1e18;
    let myWalletCEthBalance = await _balanceOf(Compound.cETH, myWalletAddress);
    let myWalletUnderlyingBalance = await _balanceOf(
      assetName,
      myWalletAddress
    );
...
const main = async () => {
  ...
  let liquidity = await _getAccountLiquidity(
    myWalletAddress,
    comptrollerAddress
  );

  console.log("Fetching cETH collateral factor...");
  let collateralFactor = await _getCollateralFactor(
    cTokenAddress,
    comptrollerAddress
  ); // as percentage
...
main().catch((err) => {
  console.error("ERROR:", err.body ? err.body.error.message : err.message);
});
